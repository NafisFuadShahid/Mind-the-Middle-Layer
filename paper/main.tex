...
\acmConference[Human-Computer Interaction]{Human-Computer Interaction}{2026}{IUT, OIC, Dhaka, Bangladesh}
...
\title[Mind the Middle Layer]{Mind the Middle Layer: A Requirements-Driven Study of Wrong Mental Models in LLM-Agent-Controlled Robots}
...
\email{akibhaider@iut-dhaka.edu}
...
\email{nafisfuad21@iut-dhaka.edu}
...
\email{sakibahmed21@iut-dhaka.edu}
...
\renewcommand{\shortauthors}{Akib, Nafis, Shanto, and Hasan Mahmud}

\begin{abstract}
People can now tell a robot what to do in plain language. Behind that request, a large language model (LLM) agent interprets it and searches a set of predefined robot functions, fixed when the robot was configured, for the ones that fit. The function set is given to the agent as context. The agent picks the matching function, the system checks the call, and the robot loads and runs it. No new code is written. The user sees none of this. They speak, and the robot moves, or it does not. We argue that this hidden middle layer leads to wrong mental models. People place intelligence in the robot instead of the agent, blame the wrong part of the pipeline when something fails, and misjudge what the robot can safely do. Because the robot acts in the physical world, these mistakes matter more than they do for a chatbot.

This is a framework-and-plan paper, not an empirical study. We contribute a stakeholder and requirements analysis; a plug-and-play architecture in which a connection handshake registers the robot's capabilities and its predefined function set, and a validation stage checks the selected function before anything is loaded; a taxonomy of seven wrong mental models drawn from the literature; a planned mixed-methods study of about one hundred participants to test that taxonomy; and mitigations tied to human-AI interaction guidelines, with an evaluation plan. We also built RoverLens, a simulator, and a scenario-based survey that tells participants where their understanding was wrong. The survey comes with an answer key, a catalogue of concrete cases for each wrong model, and a frame for reporting response statistics. No survey responses have been collected yet, and every participant example in the paper is hypothetical.
\end{abstract}

\keywords{Human-Centered AI, Interaction Design, Usability, Explainable AI,
HCI, Mental Models, Folk Theories, Human-Robot Interaction, LLMs, Function Calling}

\maketitle

\section{Introduction}

Controlling a robot used to mean writing firmware. Now a growing number of systems let a person type or speak a command to an LLM agent. The robot's configuration fixes in advance a set of functions that are already implemented for that robot (drive forward for a given time, turn, stop, switch an LED), and their descriptions are given to the agent as context. The agent works out the intent, finds the appropriate function in that set, and asks for it to be loaded or executed on the robot's microcontroller~\cite{ahn2022saycan, vemprala2024chatgpt}. The code already exists; the agent only searches for it and picks it. From the outside the exchange is short: the user talks, there is a pause, and the robot acts. Inside, several steps happen that nobody sees. The agent interprets the request, matches it to a function and its arguments, the selection is checked, and the function is loaded and run on the hardware.
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}p{0.7cm}p{1.7cm}p{7.6cm}p{4.3cm}p{1.2cm}@{}}
    ...
    \midrule
    R1 & Functional & On robot connect, the system shall auto-register the device's identity and a machine-readable capability manifest (sensors, actuators, board type) and the predefined function set, without user configuration. & Plug-and-play lowers the barrier for non-experts; grounds later refusal (A3). & Must \\
    R2 & Functional & The agent shall refuse, with an explanation, any command outside the registered capability manifest or function set, rather than attempting a partial or fabricated action. & Ungrounded LLMs otherwise produce unsafe or nonsensical actions~\cite{ahn2022saycan}; ``make clear what the system can do''~\cite{amershi2019guidelines}. & Must \\
    R3 & Quality (safety) & No selected function call shall reach the microcontroller before passing catalogue, argument-limit, and safety validation. & A physical effector makes an unchecked call a safety hazard. & Must \\
    R4 & Data & The system shall retain per-episode provenance (utterance, selected function and arguments, validation result, load status, execution feedback) to support diagnosis and study analysis. & Enables distinguishing user confusion from genuine system failure (\S8.2). & Must \\
    R5 & Quality & The interface shall report failures with a distinguishable, layer-specific cause (input, capability, function selection, loading, execution). & Users cannot otherwise localize faults (A2); supports appropriate reliance~\cite{lee2004trust}. & Must \\
    R6 & Quality & The interface shall make the hidden pipeline legible on request (e.g.\ ``choosing a function,'' ``loading,'' ``running''). & Revealing an invisible layer reshapes users' models~\cite{eslami2015always}; ``make clear why the system did what it did''~\cite{amershi2019guidelines}. & Should \\
    ...
    R8 & Organizational & When the underlying model, firmware, or function set changes, the deployer shall be able to surface a change notice to users. & Silent change violates expectations and triggers resistance~\cite{devito2017algorithms}; ``notify users about changes''~\cite{amershi2019guidelines}. & Could \\
    \bottomrule
  \end{tabular}
\end{table*}
...
Each session takes roughly 45 to 60 minutes and has five steps. (1) A baseline questionnaire covers prior experience with robots, voice assistants, and AI, and the participant's preferred language (English or Bangla), which lets us characterize the strata. (2) Participants work through \emph{scripted fault-injection scenarios}, one for each taxonomy category in \S\ref{sec:taxonomy}, in which the system fails in a controlled and pre-logged way: a disabled microphone, a request beyond the robot's capabilities, an injected function-selection error, a context-window overflow. (3) They think aloud throughout. (4) A semi-structured interview follows the scenarios. (5) A mental-model-accuracy instrument combines a short prediction task (predict the robot's behavior and the cause of a failure) with Likert items on what the person believes about the system. The RoverLens survey in \S\ref{sec:survey} is a first version of this step.
...
RoverLens simulates the pipeline in Figures~\ref{fig:arch} and~\ref{fig:flow} from start to finish. A deterministic parser stands in for the LLM and matches each command to a function in the catalogue. Command handling, validation, loading, and robot motion are all simulated, so nothing needs hardware, an account, or a network connection. The simulator runs the six stages (1~input, 2~capability check, 3~agent, 4~call validation, 5~loading, 6~execution and feedback), and a facilitator can inject a controlled fault at exactly one of them: a wrong-function selection, a missing camera, an expired context, an unavailable agent while the board stays connected, or a firmware update that cuts the maximum motion time from 5\,s to 3\,s. There are two interface conditions. They share the parser, the faults, the timing, and the animation, and differ only in what they disclose. The opaque baseline (A) shows generic progress and generic errors. The transparent condition (B) makes the requirements in Table~\ref{tab:req} visible (Table~\ref{tab:req-sim}). Each episode is logged with the command, selected function, checks, load state, feedback, injected fault, true cause, and firmware version (R4), so a failure can always be traced back to its cause.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.55cm}>{\raggedright\arraybackslash}p{3.4cm}>{\raggedright\arraybackslash}p{1.35cm}>{\raggedright\arraybackslash}p{1.65cm}@{}}
    ...
    R2 & Requests outside the manifest stop before any function is selected, with no partial action & refused, no reason & refused, with reason \\
    R3 & Catalogue, argument-limit and safety checks gate loading & enforced & enforced, shown \\
...
Existing work already covers the individual pieces of this pipeline, and we reuse them instead of reinventing them. End-user robot programming has long tried to lower the barrier to authoring robot behavior~\cite{ajaykumar2021survey}, and LLMs are the latest step. \emph{SayCan} grounds language in what a robot can actually do by selecting from a fixed set of skills, and points out that ungrounded LLMs otherwise interpret instructions in ways that are ``nonsensical or unsafe'' for the physical situation~\cite{ahn2022saycan}. Practitioner guidance on combining a high-level function library with real robot APIs also exists~\cite{vemprala2024chatgpt}. Other systems let the model write code instead of picking it. \emph{Code as Policies} showed that code-writing LLMs can generate robot control programs from natural-language commands by composing perception outputs and control-primitive APIs~\cite{liang2023code}, \emph{ProgPrompt} produces situated task plans as executable Python with precondition checks~\cite{singh2023progprompt}, and on the embedded side, generating, compiling, and flashing Arduino/ESP32 firmware from natural language is already possible with open tools~\cite{rajendra2024arduinollm}. We do not assume any of that. Generated code is imperfect in ways that users tend to miss~\cite{chen2021evaluating, perry2023users, vaithilingam2022expectation}, and a predefined function set avoids that risk, though it brings its own: the agent can still pick the wrong function, or one that does not exist.

What this earlier work does not cover is what we add, and we want to say so plainly. First, we frame the problem around users' \emph{mental models} of the whole pipeline, with a taxonomy and a study to test it. Second, we propose a \emph{plug-and-play capability handshake} that registers the robot's hardware and its predefined function set as a manifest the agent can use to refuse gracefully. Third, we place a \emph{validate-before-load} safety stage between function selection and the hardware. The function-selection and grounding mechanisms themselves come from the cited systems.

Figure~\ref{fig:arch} shows the architecture, using the module names we use throughout the paper. The central idea is that no code is written at run time. Before use, the robot is given a \emph{configuration}: a set of predefined functions (for example, drive forward for a number of seconds, turn, stop, set an LED) whose implementations already exist for the board, each with a name, a short description, its parameters, and safe limits. The \textbf{Device/Peripheral layer} auto-detects the robot and runs a configuration handshake when it connects (similar to USB HID enumeration or OS driver binding), which registers the hardware and this function set as the capability manifest (R1). The function descriptions are supplied to the agent as context. The \textbf{Input/Perception layer} handles the microphone, camera, and sensors, and if a required sensor is missing it reports that specific absence instead of carrying on (R5). The \textbf{Agent/Reasoning layer} parses intent and searches the function set in its context for the function, or sequence of functions, that fits, together with its arguments. It can run on a local, on-device (fine-tuned) model or on a remote API model. With a remote model, a \textbf{connectivity handler} manages retries, timeouts, an offline fallback, and a latency budget. The \textbf{Call Validation layer} checks that the selected function exists in the set, that its arguments are within the configured limits, and that the call passes safety checks before anything is loaded (R3). The \textbf{Loading layer} loads the selected function on the microcontroller, or triggers it if it is already resident. Nothing is compiled or written, so toolchain differences between boards (ESP32 versus classic Arduino) show up only in which functions a configuration provides. The \textbf{Execution \& Feedback layer} confirms whether the action happened and reports back to the agent and the user (R5, R6).
...
\begin{tikzpicture}[>=Stealth]
  ...
  \node[hid, tag, text width=2.05cm] (agent) at (5.6,0)  {\dgtitle{Agent / Reasoning}\\ intent, function lookup \nodepart{two}\wmtag{WM1 $\cdot$ WM4 $\cdot$ WM6}};
  \node[hid, tag, text width=2.05cm] (valid) at (8.4,0)  {\dgtitle{Call Validation}\\ exists, limits, safety \nodepart{two}\wmtag{WM2}};
  \node[hid, tag, text width=2.05cm] (deploy) at (11.2,0) {\dgtitle{Loading}\\ load + run function \nodepart{two}\wmtag{WM2 $\cdot$ WM7}};
  ...
  \node[store, text width=2.5cm] (manifest) at (6.4,-4.0)  {\dgtitle{Capability manifest}\\ sensors, actuators,\\ board, function set};
...
\begin{tikzpicture}[>=Stealth, x=1cm, y=1cm,
  ...
  \node[dec] (d4) at (11.25,0) {Call valid \& safe?};
  \node[dec] (d5) at (14.15,0) {Loaded OK?};
  ...
  \node[bad, tag, text width=1.85cm] (f4) at (11.25,-2.6) {Block load; report invalid call \nodepart{two}\wmtag{WM2}};
  \node[bad, tag, text width=1.85cm] (f5) at (14.15,-2.6) {Report loading failure \nodepart{two}\wmtag{WM2}};
  ...
  \foreach \i/\t/\c in {1/{Input / Perception}/cVisD, 2/{Capability check}/cHidD, 3/{Agent / Reasoning}/cHidD,
                        4/{Call Validation}/cHidD, 5/{Loading}/cHidD, 6/{Execution \& Feedback}/cHwD} {
  ...
  \draw[main] (d3) -- node[above=1.4mm, lab, align=center] {yes: pick\\ function (LLM)} (d4);
  ...
  \node[lab, anchor=north west, text width=10.5cm, align=left] at ([yshift=-4.2mm]b1.south west)
       {\textbf{Categories with no failure exit:} WM1 (uneven language or board competence in the agent's matching), WM5 (over-trust after early success) and WM7 (silent model or firmware change) shape what the user expects, not which exit is taken.};
...
\paragraph{WM1: uneven language or function-matching competence read as ``the robot is broken'' or ``it got dumber.''}
Model performance is uneven across input languages and target boards. LLMs do markedly worse in lower-resource languages than in high-resource ones~\cite{joshi2020state}, and the function set can differ across boards and configurations. Picture the same instruction working in English but degrading in Bangla, or working on an Arduino Uno configuration but not on an ESP32 one whose function set lacks a matching function. The user cannot see how the agent matches a request to the language- or configuration-specific function set, so the difference gets blamed on the robot (``broken,'' ``dumber today'') instead of on gaps in the hidden agent.

\paragraph{WM2: an undiagnosed fault treated as a single, permanent failure.}
A failed command can start in the microphone, the motors, the selected function, or a thermal shutdown. Each needs a different fix, and none is permanent. Say the robot does not move. The real cause is an injected function-selection error (the agent picked a function that is not in the set), the microphone light was on, and the motors are fine. Without layer-specific feedback the user folds these distinct faults into one verdict (``it doesn't work'') and may throw away hardware that works. The grounding literature notes that ungrounded LLM output is itself a common and non-obvious source of failure~\cite{ahn2022saycan}, and appropriate reliance requires that users can tell capable states from incapable ones~\cite{lee2004trust}. Trust in a robot drops with observed failures and depends on the feedback it gives about them~\cite{desai2013impact, salem2015would}, and HRI already has a vocabulary for failure types and how to resolve them~\cite{honig2018understanding}.
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}p{0.7cm}p{3.3cm}p{4.0cm}p{4.3cm}p{2.3cm}@{}}
    ...
    \midrule
    WM1 & ``Robot is broken / got dumber'' & Language- or board-dependent function-matching gap & Hidden, language/configuration-specific matching invisible & \cite{joshi2020state} \\
    WM2 & One permanent failure & Distinct fault sources (mic, motor, function selection, thermal) & No layer-specific feedback & \cite{ahn2022saycan, lee2004trust} \\
    ...
    WM7 & ``It used to work'' & Silent model, firmware, or function-set update changed behavior & Stable model of an assumed-static system & \cite{devito2017algorithms, amershi2019guidelines} \\
    \bottomrule
  \end{tabular}
\end{table*}
...
\begin{itemize}
  ...
  \item \textbf{Rifat, 27, CS faculty.} Facing a silent no-move (WM2), she \emph{could} guess ``dead motor'' when the injected fault was in the function the agent picked, which would illustrate WM2.
  \item \textbf{Mueeze, 22, CS student.} Asked for an action that needs an absent sensor (WM3), he \emph{might} accept a partial motion as ``almost worked,'' plausibly WM3.
  \item \textbf{Mrs.\ Ahmed, 45, English-first, non-technical.} After a failed follow-up (WM4), she \emph{could} say the robot ``ignored'' her, consistent with WM4.
  \item \textbf{Faisal, 31, expert developer.} After several successes he \emph{might} hand over a riskier command unchecked (WM5), and separately \emph{might} remark that ``it behaved differently than last week'' after an unannounced update (WM7).
\end{itemize}
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}p{0.9cm}p{4.2cm}p{5.4cm}p{3.6cm}@{}}
    ...
    \midrule
    WM1 & Vignette only (RoverLens accepts English) & Language coverage of the agent's matching, not hardware & \texttt{wm1\_why} \\
    WM2 & \emph{Where did it fail?} Send ``Move forward for 2 seconds'' & Agent picks a function that is not in the catalogue; fails call validation; nothing loaded; motors fine & \texttt{wm2\_stage}, \texttt{wm2\_cause} \\
    WM3 & \emph{A capability boundary.} ``Find the red ball'' & Manifest has no camera; refused at the capability check before any function is selected & \texttt{wm3\_can} \\
    WM4 & \emph{A missing memory.} Full command, then ``Do that again'' & Context expired before the follow-up; fails at the agent stage; no function selected & \texttt{wm4\_why}, \texttt{wm4\_where} \\
    ...
    WM6 & \emph{Who does the thinking?} ``Turn left'' & External agent unavailable while the board stays connected; no new function selected & \texttt{wm6\_where}, \texttt{wm6\_why} \\
...
\input{paper/generated/cases.tex}
...
\begin{itemize}
  \item \textbf{Layer-specific status indicators} that separate input, capability, function-selection, loading, and execution failures (targets WM2; R5; guideline: support efficient recovery).
  ...
  \item \textbf{Pipeline-legibility cues} (``choosing a function,'' ``loading,'' ``running'') that reveal the hidden layer on request (WM6; R6; revealing an invisible layer reshapes models~\cite{eslami2015always}, and legible behavior lets an observer infer intent~\cite{dragan2013legibility}).
...
\input{paper/generated/instrument.tex}
